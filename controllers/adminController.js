...
function get(req, res) {
    ...
    let query = `SELECT * FROM admin where email="${req.body.data.email}" and password="${password}"`;
    ...
    sql.query(query, (err, result) => {
        if (err) {
            ...
            if(result.length > 0){
                // retData = {success: true, message: 'Password matched', accessToken: '', userData: result};
                retData.success = true;
                retData.message = 'Password matched';
                retData.accessToken = '';
                retData.userData = result;
